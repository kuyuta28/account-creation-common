...
# SUPPORTED_SERVICES sẽ được inject qua init_db() hoặc set từ bên ngoài
# Mặc định là empty set — service nào cần sẽ set khi gọi init_db
_SUPPORTED_SERVICES: set[str] = set()


def set_supported_services(services: set[str]) -> None:
    """Set supported services list — gọi từ service startup."""
    global _SUPPORTED_SERVICES
    _SUPPORTED_SERVICES = {s.upper() for s in services}


def _service_has_registrar(service: str) -> bool:
    return service.upper() in _SUPPORTED_SERVICES
