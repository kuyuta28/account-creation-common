...
# ── Config Protocols (thay vì import từ service-specific settings) ────────────

@runtime_checkable
class LogConfig(Protocol):
    """Protocol cho log config — tương thích với mọi service's settings."""
    console: "ConsoleLogConfig"
    file: "FileLogConfig"


@runtime_checkable
class ConsoleLogConfig(Protocol):
    enabled: bool
    level: str
    format: str
...
@runtime_checkable
class AllLogConfig(Protocol):
    """Protocol cho all-log tee config."""
    enabled: bool
    path: str
    append: bool
...
def make_logger(
    ...
    log_cfg  — LogConfig protocol instance (tương thích với mọi service's settings)
